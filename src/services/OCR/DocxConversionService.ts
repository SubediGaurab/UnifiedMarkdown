...
export class DocxConversionService implements IOCRService {
    ...
    async extractText(docxPath: string): Promise<void> {
        try {
            ...
            if (extractedImages.length > 0) {
                logger.info(`Found ${extractedImages.length} image(s), generating captions...`);
            ...
            if (extractedCharts.length > 0) {
                logger.info(`Generating descriptions for ${extractedCharts.length} chart(s)...`);

                const chartResults = await Promise.all(
                    extractedCharts.map(async (chart) => ({
                        index: chart.index,
                        description: await this.generateChartDescription(chart.xmlContent, summary),
                    }))
                );
            ...
            if (chartDescriptionsHtml) {
                ...
                if (tableEndMatch && tableEndMatch.index !== undefined) {
                    ...
                    html = html.slice(0, insertPos) + chartDescriptionsHtml + html.slice(insertPos);
                } else {
                    ...
                    if (headingMatch && headingMatch.index !== undefined) {
                        ...
                        html =
                            html.slice(0, insertPos) + chartDescriptionsHtml + html.slice(insertPos);
                    } else {
    ...
    private async generateDocumentSummary(text: string): Promise<string> {
        try {
            ...
            logger.warning(`Failed to generate summary, using truncated text: ${error}`);
